...
public class PhotoHandler : IPhotoHandler
{
    ...
    public PhotoHandler(EFContext context, IBlobService blob, IMapper mapper)
    {
        _context = context;
    ...
    public async Task<MessageDto> UploadAsync(PhotoPostParams @params)
    {
        ...
        try
        {
            ...
            var url = await _blob.UploadBlobFileAsync(EContainer.Photos, @params.File, photoId, hash);
    ...
    public async Task<MessageDto> UpdateAsync(PhotoPutParams @params)
    {
        ...
        try
        {
            var obj = await _photo.AsNoTracking().FirstOrDefaultAsync(x => x.Id == @params.Id);
            ...
            {
                obj.Url = await _blob.UpdateBlobFileAsync(EContainer.Photos, @params.File, obj.Id, obj.HashCode);
            }
    ...
    public async Task<MessageDto> DeleteAsync(DeleteParams @params)
    {
        ...
        var userPhotosToDelete = await _userPhoto.AsNoTracking().Where(x => lstId.Contains(x.Id)).ToListAsync();
        ...
        try
        {
            ...
            await _blob.DeleteBlobFileAsync(EContainer.Photos, filesToDelete);
