...
public class ScriptHandler : IScriptHandler
{
    ...
    public ScriptHandler(EFContext context, IBlobService blob, IMapper mapper)
    {
        _context = context;
    ...
    public async Task<MessageDto> UploadAsync(ScriptPostParams @params)
    {
        ...
        try
        {
            ...
            var url = await _blob.UploadBlobFileAsync(EContainer.Scripts, @params.File, scriptId, hash);
    ...
    public async Task<MessageDto> UpdateAsync(ScriptPutParams @params)
    {
        ...
        try
        {
            var obj = await _script.AsNoTracking().FirstOrDefaultAsync(x => x.Id == @params.Id);
            ...
            {
                obj.Url = await _blob.UpdateBlobFileAsync(EContainer.Scripts, @params.File, obj.Id, obj.HashCode);
            }
    ...
    public async Task<MessageDto> DeleteAsync(DeleteParams @params)
    {
        ...
        var userScriptsToDelete = await _userScript.AsNoTracking().Where(x => lstId.Contains(x.Id)).ToListAsync();
        ...
        try
        {
            ...
            await _blob.DeleteBlobFileAsync(EContainer.Scripts, filesToDelete);
