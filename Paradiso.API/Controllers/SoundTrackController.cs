...
public class SoundTrackController : ControllerBase
{
    ...
    [HttpPost("SoundTrack")]
    public async Task<IActionResult> UploadAsync([FromForm] SoundTrackPostParams @params) => Ok(await _handler.UploadAsync(@params));

    [HttpPut("SoundTrack")]
    public async Task<IActionResult> UpdateAsync([FromForm] SoundTrackPutParams @params) => Ok(await _handler.UpdateAsync(@params));
